...
import { Loader2, ChevronRight, Zap, BookOpen, List, Star, GitCompare, HelpCircle } from 'lucide-react';
      ...
      <div>
        ...
        <div className="grid grid-cols-5 gap-3">
          {articleTypes.map((t) => (
            ...
            >
              <div className={cn('mb-2', articleType === t.value ? 'text-blue-400' : 'text-gray-500')}>
                {t.icon}
              </div>
              <p className={cn('text-sm font-semibold mb-1', articleType === t.value ? 'text-white' : 'text-gray-300')}>
                {t.label}
              </p>
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div>
          ...
          <input
            type="number"
            value={targetLength}
            onChange={(e) => setTargetLength(parseInt(e.target.value) || 2000)}
            min={500}
            step={100}
      ...
      <div className="flex items-center gap-8">
        {[
          ...
          <button
            ...
            onClick={() => set(!value)}
            className={cn(
              'flex items-center gap-3 px-4 py-2.5 rounded-xl border-2 transition-all text-sm',
              value
          ...
          >
            ...
            {label}
            <div className={cn(
              'w-8 h-4 rounded-full border-2 flex items-center transition-all ml-1',
      ...
      <div>
        ...
        <div className="grid grid-cols-4 gap-3">
          {tones.map((t) => (
            ...
            >
              <p className={cn('text-sm font-semibold mb-1', tone === t.value ? 'text-violet-300' : 'text-gray-300')}>
                {t.label}
              </p>
              <p className="text-xs text-gray-500">{t.desc}</p>
            </button>
          ...
          {aiModels.map((m) => (
            <button
              ...
              className={cn(
                'p-4 rounded-xl border-2 text-left transition-all',
                aiModel === m.value
                  ? 'border-blue-500 bg-gradient-to-br from-blue-500/10 to-transparent'
                  : 'border-gray-700 hover:border-gray-600 bg-gray-800/50'
              )}
            >
              <div className={cn(
              ...
              <p className={cn('text-sm font-bold mb-1', aiModel === m.value ? 'text-white' : 'text-gray-300')}>
                {m.label}
              </p>
